import { generateScrum } from "./core/scrum.service.js";
import { renderNormalizedScrum } from "./ui/renderer.js";
import {
  formatDate,
  escapeHtml,
  compactTextStyle,
...
function allIncluded(outputTarget = "email") {
  ...
  if (
    platform === "gitlab" ||
    (typeof platform === "undefined" && window.GitLabHelper)
  ) {
  ...
  function getChromeData() {
    ...
    browser.storage.local
      ...
      .then((items) => {
        ...
        console.log(
          `[DEBUG] platform: ${platform}, platformUsername: ${platformUsername}`,
        );

        if (outputTarget === "popup") {
          const usernameFromDOM =
        ...
        if (onlyIssues && onlyPRs) {
          console.warn(
            "[SCRUM-HELPER]: Detected both onlyIssues and onlyPRs enabled; normalizing to onlyIssues.",
          );
          onlyPRs = false;
        ...
        if (onlyMergedPRs) {
          ...
          if (onlyRevPRs) {
            console.warn(
              "[SCRUM-HELPER]: onlyMergedPRs and onlyRevPRs both enabled; disabling onlyRevPRs.",
            );
            onlyRevPRs = false;
          ...
          if (onlyIssues) {
            console.warn(
              "[SCRUM-HELPER]: onlyMergedPRs and onlyIssues both enabled; disabling onlyIssues.",
            );
            onlyIssues = false;
          ...
          if (onlyPRs) {
            console.warn(
              "[SCRUM-HELPER]: onlyMergedPRs and onlyPRs both enabled; disabling onlyPRs.",
            );
            onlyPRs = false;
              ...
              (async () => {
                try {
                    ...
                    return {
                      ...
                      html_url:
                        type === "issue"
                          ? item.web_url ||
                            (project
                              ? `${project.web_url}/-/issues/${item.iid}`
                              : "")
                          : item.web_url ||
                            (project
                              ? `${project.web_url}/-/merge_requests/${item.iid}`
                              : ""),
                      number: item.iid,
                      title: item.title,
                      state:
                        type === "issue" && item.state === "opened"
                          ? "open"
                          : item.state,
                      project: repoName,
                  ...
                  const mappedIssues = (data.issues || []).map((issue) =>
                    mapGitLabItem(issue, data.projects, "issue"),
                  );
                  const mappedMRs = (data.mergeRequests || data.mrs || []).map(
                    (mr) => mapGitLabItem(mr, data.projects, "mr"),
                  );
                  const mappedData = {
  ...
  async function fetchGithubData() {
    ...
    selectedRepos = Array.isArray(filterSettings.selectedRepos)
      ? filterSettings.selectedRepos
      : [];
    ...
    if (
      githubCache.fetching ||
      (githubCache.cacheKey === cacheKey && githubCache.data)
    ) {
    ...
    if (githubCache.data && isCacheFresh && isCacheKeyMatch) {
      if (needsToken && !cacheUsedToken) {
        log(
          "Cache was fetched without token, but user now has a token. Invalidating cache.",
        );
      ...
      const repoQueries = selectedRepos
        ...
        .map((repo) => {
          if (typeof repo === "object" && repo.fullName) {
            const cleanName = repo.fullName.startsWith("/")
              ? repo.fullName.substring(1)
              : repo.fullName;
          ...
          const fullRepoInfo = githubCache.repoData?.find(
            (r) => r.name === repo,
          );
    ...
    try {
      ...
      if (userCheckRes.status === 404) {
        const errorMsg =
          chrome?.i18n.getMessage("githubUserNotFoundError", [
            platformUsernameLocal,
          ]) ||
      ...
      if (!userCheckRes.ok) {
        const errorMsg =
          chrome?.i18n.getMessage("githubUserValidationError", [
            userCheckRes.status,
            userCheckRes.statusText,
          ]) ||
      ...
      if (!issuesRes.ok) {
        const errorMsg =
          chrome?.i18n.getMessage("githubIssuesFetchError", [
            issuesRes.status,
            issuesRes.statusText,
          ]) ||
      ...
      if (!prRes.ok) {
        const errorMsg =
          chrome?.i18n.getMessage("githubPRReviewFetchError", [
            prRes.status,
            prRes.statusText,
          ]) ||
      ...
      if (!userRes.ok) {
        const errorMsg =
          chrome?.i18n.getMessage("githubUserFetchError", [
            userRes.status,
            userRes.statusText,
          ]) ||
      ...
      if (githubIssuesData && githubIssuesData.items) {
        log(
          "Fetched githubIssuesData:",
          githubIssuesData.items.length,
          "items",
        );
        ...
        if (openPRs.length && githubToken && showCommits) {
          ...
          openPRs.forEach((pr) => {
            ...
            log(
              `Attached ${pr._allCommits.length} commits to PR #${pr.number}`,
            );
            if (pr._allCommits.length > 0) {
              log(
                `Commits for PR #${pr.number}:`,
                pr._allCommits.map(
                  (c) => `${c.messageHeadline} (${c.committedDate})`,
                ),
              );
            }
          });
      ...
      if (outputTarget === "popup") {
        ...
        if (scrumReport) {
          let errorMsg =
            chrome?.i18n.getMessage("reportGenerationError") ||
  ...
  async function fetchCommitsForOpenPRs(prs, githubToken, startDate, endDate) {
    ...
    prs.forEach((pr, idx) => {
      const prData =
        data.data && data.data[`pr${idx}`] && data.data[`pr${idx}`].pullRequest;
      if (prData && prData.commits && prData.commits.nodes) {
        ...
        const filteredCommits = allCommits.filter((commit) => {
          ...
          log(
            `PR #${pr.number} commit "${commit.messageHeadline}" (${commit.committedDate}) - in range: ${isInRange}`,
          );
          return isInRange;
        });
  ...
  async function processGithubData(data) {
    ...
    log(
      "[SCRUM-DEBUG] Processing issues for main activity:",
    ...
    log(
      "[DEBUG] Both data processing functions completed, generating scrum body",
    );
    if (subjectForEmail) {
  ...
  function buildActivityListHtml() {
    if (lastWeekArray.length === 0 && reviewedPrsArray.length === 0) {
      return wrapCompactText(
        "No activity to report for the selected time period.",
      );
    }

    let activityList = "<ul>";
    for (let i = 0; i < lastWeekArray.length; i++)
      activityList += lastWeekArray[i];
    for (let i = 0; i < reviewedPrsArray.length; i++)
      activityList += reviewedPrsArray[i];
    activityList += "</ul>";
    return activityList;
  }
  ...
  function buildNextWeekListHtml() {
    ...
    let nextWeekList = "<ul>";
    for (let i = 0; i < nextWeekArray.length; i++)
      nextWeekList += nextWeekArray[i];
    nextWeekList += "</ul>";
    return nextWeekList;
  }
...
${blockerText}`;
    ...
    if (outputTarget === "popup") {
      ...
      const observer = new MutationObserver((mutations, obs) => {
        ...
        if (window.emailClientAdapter.isNewConversation()) {
          ...
          if (elements && elements.body) {
            obs.disconnect();
            log(
              "MutationObserver found the editor body. Injecting scrum content.",
            );
            window.emailClientAdapter.injectContent(
              elements.body,
              content,
      ...
      setTimeout(() => {
        ...
        if (!hasInjectedContent && scrumGenerationInProgress) {
          logError(
            "Injection timed out after 30 seconds. The compose window might not have loaded.",
          );
          scrumGenerationInProgress = false;
        }
      }, 30000);
  ...
  function writeGithubPrsReviews() {
    ...
    log("Filtering PR reviews by date range:", {
      startDate,
      endDate,
      startDateTime,
      endDateTime,
    ...
    for (i = 0; i < items.length; i++) {
      ...
      if (platform === "github") {
        isAuthoredByUser =
          item.user && item.user.login === platformUsernameLocal;
      } else if (platform === "gitlab") {
        isAuthoredByUser =
          item.author && item.author.username === platformUsername;
      }
      ...
      log(
        `PR #${item.number} - itemDate: ${itemDate}, startDateTime: ${startDateTime}, endDateTime: ${endDateTime}`,
      );
      if (itemDate < startDateTime || itemDate > endDateTime) {
        log(
          `Skipping PR #${item.number} - updated at ${itemDate} outside date range ${startDate} to ${endDate}`,
        );
        continue;
      }
      ...
      if (createdDate < startDateTime) {
        ...
        if (itemDate < lastDayOfRange) {
          log(
            `Skipping PR #${item.number} - created before date range and not updated recently enough`,
          );
          continue;
        }
      }
      ...
      if (yesterdayContribution) {
        ...
        const wasCreatedYesterday =
          createdDate >= yesterday && createdDate <= today;
      ...
      const project = repository_url.substr(
        repository_url.lastIndexOf("/") + 1,
      );
      ...
      if (githubPrsReviewDataProcessed[repo].length <= 1) {
        for (const pr in githubPrsReviewDataProcessed[repo]) {
          ...
          prText +=
            ...
            pr_arr.title +
            ") ";
          if (showOpenLabel && pr_arr.state === "open")
        ...
        for (const pr1 in githubPrsReviewDataProcessed[repo]) {
          ...
          prText1 +=
            ...
            pr_arr1.title +
            ") ";
          if (showOpenLabel && pr_arr1.state === "open")
  ...
  async function writeGithubIssuesPrs(items) {
    ...
    if (githubToken) {
      ...
      if (prsToCheck.length > 0) {
        mergedStatusResults = await fetchPrsMergedStatusBatch(
          prsToCheck,
          headers,
        );
      }
      ...
      if (prsToCheck.length > 30) {
        ...
        for (const pr of prsToCheck) {
          const merged = await fetchPrMergedStatusREST(
            pr.owner,
            pr.repo,
            pr.number,
            headers,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isAnyFilterActive) {
        if (isMR && !onlyPRs) {
          log(
            "[SCRUM-DEBUG] Filters active, skipping PR because onlyPRs is not checked:",
            item.number,
          );
          continue;
        }
        if (!isMR && !onlyIssues) {
          log(
            "[SCRUM-DEBUG] Filters active, skipping Issue because onlyIssues is not checked:",
            item.number,
          );
          continue;
        }
      ...
      if (onlyMergedPRs) {
        if (!isMR) {
          log(
            "[SCRUM-DEBUG] Skipping non-PR item because onlyMergedPRs is checked:",
            item.number,
          );
          continue;
        }
        if (isMR) {
          ...
          if (prCacheKey && prCacheKey in mergedStatusResults) {
            ...
            item.pull_request &&
            Object.prototype.hasOwnProperty.call(item.pull_request, "merged_at")
          ) {
            hasMergeInfo = true;
      ...
      if (isMR) {
        ...
        if (yesterdayContribution) {
          ...
          endDateFilter = new Date(
            today.toISOString().split("T")[0] + "T23:59:59Z",
          ); // Use yesterday for start and today for end
        } else if (startingDate && endingDate) {
        ...
        const isNewPR =
          prCreatedDate >= startDateFilter && prCreatedDate <= endDateFilter;
        const prUpdatedDate = new Date(item.updated_at);
        const isUpdatedInRange =
          prUpdatedDate >= startDateFilter && prUpdatedDate <= endDateFilter;

        // Check if PR has commits in the date range
        const hasCommitsInRange =
          item._allCommits && item._allCommits.length > 0;
        ...
        if (platform === "github") {
          ...
          if (!isNewPR && !onlyMergedPRs) {
            if (item.state !== "open") {
              log(
                `[PR DEBUG] Skipping PR #${number} - existing PR but not open`,
              );
              continue;
            }
            if (!hasCommitsInRange) {
              log(
                `[PR DEBUG] Skipping PR #${number} - existing PR but no commits in date range`,
              );
              continue;
            }
          }
        ...
        if (isDraft) {
          ...
          if (
            showCommits &&
            item._allCommits &&
            item._allCommits.length &&
            !isNewPR
          ) {
            log(
              `[PR DEBUG] Rendering commits for existing draft PR #${number}:`,
              item._allCommits,
            );
            li += "<ul>";
          ...
          if (
            ...
            log(
              `[PR DEBUG] Rendering commits for existing PR #${number}:`,
              item._allCommits,
            );
            li += "<ul>";
            item._allCommits.forEach((commit) => {
...
<span style="color:#2563eb;">${commit.messageHeadline}</span><span style="color:#666; font-size: 11px;"> (${new Date(commit.committedDate).toLocaleString()})</span></li>`;
        ...
        if (
          item.state === "open" &&
          item.body?.toUpperCase().indexOf("YES") > 0
        ) {
          const li2 =
            "<li><i>(" +
            project +
        ...
        itemCreatedDate.setHours(0, 0, 0, 0);
        const isCreatedToday = today.getTime() === itemCreatedDate.getTime();
        const issueActionText = isCreatedToday
          ? "Opened Issue"
          : "Updated Issue";
        if (item.state === "open") {
  ...
  const intervalSubject = setInterval(() => {
    const userData =
      platform === "gitlab"
        ? githubUserData || platformUsername
        : githubUserData;
    ...
    if (
      outputTarget === "email" &&
      !window.emailClientAdapter.isNewConversation()
    ) {
  ...
  const intervalWriteGithubIssues = setInterval(() => {
    ...
    const username =
      platform === "gitlab" ? platformUsername : platformUsernameLocal;
  ...
  if (!refreshButton_Placed) {
    const intervalWriteButton = setInterval(() => {
      if (
        document.getElementsByClassName("F0XO1GC-x-b").length === 3 &&
        scrumBody
      ) {
        ...
        document
          .getElementsByClassName("F0XO1GC-x-b")[0]
          .children[0].children[0].appendChild(td);
        document
...
async function injectIntoEmailEditor(content, subject) {
  ...
  const tryInject = () => {
    ...
    if (subject && elements.subject) {
      ...
      elements.subject.dispatchEvent(
        new Event(elements.eventTypes?.subjectChange || "input", {
          bubbles: true,
        }),
      );
    }
...
async function fetchPrsMergedStatusBatch(prs, headers) {
  ...
  if (
    ...
    graphqlHeaders.Authorization.startsWith("token ")
  ) {
    graphqlHeaders.Authorization = graphqlHeaders.Authorization.replace(
      "token ",
      "bearer ",
    );
  }
...
async function fetchUserRepositories(username, token, org = "") {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Using date range for repo search: ${startDate} to ${endDate}`,
      );
    } catch (err) {
    ...
    const [issuesRes, commentsRes] = await Promise.all([
      fetch(issuesUrl, { headers }).catch(() => ({
        ok: false,
        json: () => ({ items: [] }),
      })),
      fetch(commentsUrl, { headers }).catch(() => ({
        ok: false,
        json: () => ({ items: [] }),
      })),
    ]);
    ...
    if (issuesRes.ok) {
      ...
      console.log(
        `Found ${issuesData.items?.length || 0} issues/PRs authored by user in date range`,
      );
    }
    ...
    if (commentsRes.ok) {
      ...
      console.log(
        `Found ${commentsData.items?.length || 0} issues/PRs with user comments in date range`,
      );
    }

    const repoNames = Array.from(repoSet);
    console.log(
      `Found ${repoNames.length} unique repositories with contributions in the selected date range`,
    );

    if (repoNames.length === 0) {
      console.log(
        `No repositories with contributions found in the selected date range`,
      );
      return [];
    }
...
async function generateScrumFromProvider() {
  try {
    ...
    const activeToken = items.githubToken || githubToken;
    const activeUsername = items.githubUsername || platformUsername;
    const activeStartDate = items.startingDate || startingDate;
    const activeEndDate = items.endingDate || endingDate;
    ...
    //  ONLY CHANGE: use CORE instead of provider directly
    const data = await generateScrum({
      platform: "github",
      config: {
        token: activeToken,
    ...
    console.error("Provider flow failed:", err);
  }
}

window.generateScrumFromProvider = generateScrumFromProvider;
